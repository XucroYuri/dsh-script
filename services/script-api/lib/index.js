...
export const SCRIPT_STUDIO_API_VERSION = '1.0.0';
...
function failure(requestId, status, cause) {
...
export class ScriptStudioApi {
    sessions;
    hierarchy;
    constructor(sessions, hierarchy) {
    ...
    async handle(request) {
        ...
        if (!match)
            return failure(request.requestId, 404, new DomainError('not-found', 'Script Studio API route was not found.'));
        ...
        let session;
        try {
            session = await this.sessions.verify(accessToken);
        }
        catch {
            return unauthorized(request.requestId);
        }
        ...
        let projectId;
        try {
            projectId = asProjectId(decodeURIComponent(match[1]));
        }
        catch (cause) {
